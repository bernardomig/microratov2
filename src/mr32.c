...
#define DIM_BAT_ARRAY 64 // Dimension of the battery voltage array
  ...
  unsigned int readLineSensors(int gain)
  {
    ...
    LATECLR = 0x0020; // Disable line sensor
    LATD =
      LATD | 0x00EC; // All 5 outputs set (just in case, set in initPIC32() )
  ...
  //
  unsigned int batteryVoltage(void) { return analogSensors.array[BATTERY]; }
  ...
  void closedLoopControl(BOOL flag) { _closedLoopControl = flag; }
