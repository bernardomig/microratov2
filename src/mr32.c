...
// local functions
void
pid(int sp_m1, int enc_m1, int sp_m2, int enc_m2);
void
readEncoders(int* enc_m1, int* enc_m2);
void
actuateMotors(int pwmL, int pwmR);
unsigned int
updateBatteryVoltage(int value);
void
setPWM2(int pwmL, int pwmR);
void
setSP2(int spL, int spR);
...
#define SERVO_K                                                                \
  (((SERVO_WIDTH_MAX - SERVO_WIDTH_MIN) * T2_FREQ) / 1000) / SERVO_LEVELS
...
void
getRobotPos(double* xx, double* yy, double* tt)
{
  *xx = xpos;
  *yy = ypos;
  *tt = theta;
...
//
unsigned int
obstacleSensor(unsigned int sensorId)
{
  if (sensorId > 2)
    sensorId = 2;
...
//
void
readEncoders(int* encLeft, int* encRight)
{
  DisableInterrupts();
  *encLeft = -counterLeft;
...
// PID controller.
//
// THIS FUNCTION SHOULD BE CALLED FROM A TIMER INTERRUPT SERVICE ROUTINE (T >=
// 10ms) DO NOT CALL THIS FUNCTION DIRECTLY FROM YOUR CODE (MAIN). DOING THAT
// MAY PERMANENTLY DAMAGE THE MOTOR DRIVE.
//
void
pid(int spMLeft, int encMLeft, int spMRight, int encMRight)
{
  ...
  integralMLeft =
    integralMLeft < -INTEGRAL_CLIP ? -INTEGRAL_CLIP : integralMLeft;
  ...
  integralMRight =
    integralMRight > INTEGRAL_CLIP ? INTEGRAL_CLIP : integralMRight;
  integralMRight =
    integralMRight < -INTEGRAL_CLIP ? -INTEGRAL_CLIP : integralMRight;
...
void
updateLocalization(int encLeft, int encRight)
{
...
//
void
wait(unsigned int tenth_seconds)
{
  resetCoreTimer();
...
void _int_(_TIMER_2_VECTOR) isr_t2(void)
{
  ...
  if ((cntT2Ticks % 2) == 0)
    tick20ms = 1; // Set every 20 ms
  if ((cntT2Ticks % 4) == 0)
    tick40ms = 1; // Set every 40 ms
  if ((cntT2Ticks % 8) == 0)
    tick80ms = 1; // Set every 80 ms
  if ((cntT2Ticks % 16) == 0)
    tick160ms = 1; // Set every 160 ms
  ...
  if (_closedLoopControl == TRUE) {
    ...
    if (cntT2Ticks & 0x01) // This way SAMPLING_T is 20 ms
    {
      pid(spLeft,
          encLeftAcc,
          spRight,
          encRightAcc); // spLeft, spRight are global vars
