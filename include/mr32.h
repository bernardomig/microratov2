...
#define enableObstSens()                                                       \
  {                                                                            \
    LATBbits.LATB10 = 1;                                                       \
  }
#define disableObstSens()                                                      \
  {                                                                            \
    LATBbits.LATB10 = 0;                                                       \
  }

#define enableGroundSens()                                                     \
  {                                                                            \
    LATEbits.LATE5 = 1;                                                        \
  }
#define disableGroundSens()                                                    \
  {                                                                            \
    LATEbits.LATE5 = 0;                                                        \
  }
 ...
 */
unsigned int
readLineSensors(int gain); // for default value of gain, pass 0
 ...
 * \details velocities range from -100 to 100, where 100 roughly represents 0.7
 * m/s \ingroup actuators \param velLeft setpoint for left motor, in range
 * [-100, 100] \param velRight setpoint for right motor, in range [-100, 100]
 */
void
setVel2(int velLeft, int velRight);
